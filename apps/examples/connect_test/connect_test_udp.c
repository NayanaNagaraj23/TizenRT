...
static int _data_transfer(int transfer_mode, int fd, int size, struct sockaddr_in *addr)
{
	...
	FD_SET(fd, &ofds);
	bool isError = false;
	DECLARE_TIMEOUT CT_LOG(TAG, "DATA TRANSFER START");
...
int _udp_server(int msg_size, long *sum)
{
	...
	TERMINATE strncpy((char *)g_recv_mac_buf_server, g_buf, MAC_BUF_SIZE);
...
int _udp_client(char *server_addr, int msg_size)
{
	...
	TERMINATE CT_LOG(TAG, "Receiving MAC from peer");
	ret = _data_transfer(1, sockfd, MAC_BUF_SIZE, &saddr);
	TERMINATE strncpy((char *)g_recv_mac_buf_client, g_buf, MAC_BUF_SIZE);
