...
static int mbedtls_seclink_aes_self_test(int verbose)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (verbose != 0)
			printf(" AES-ECB-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
	...
	for (i = 0; i < 6; i++) {
		...
		if (verbose != 0)
			printf(" AES-CBC-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
	...
	for (i = 0; i < 6; i++) {
		...
		if (verbose != 0)
			printf(" AES-CFB128-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
...
END_TEST_F START_TEST_F(ecdsa_selftest)
{
	ST_ASSERT_EQ(0, 0);
}

END_TEST_F void sl_handle_mbed_aes(sl_options * opt)
{
...
{
	SL_PARSE_MESSAGE(opt, g_command, sl_mbed_type_e, g_func_list, SL_MBED_TYPE_MAX, SL_MBED_TYPE_ERR);
	ST_RUN_TEST(sl_mbed);
