...
void wm_sta_disconnected((wifi_manager_cb_msg_s msg, void *arg) {
						 ...
						 WM_TEST_SIGNAL; return;}
						 wifi_manager_scan_info_s * wifi_scan_iter = msg.scanlist; while (wifi_scan_iter != NULL) {
						 printf("WiFi AP SSID: %-25s, BSSID: %-20s, Rssi: %d, Auth: %d, Crypto: %d\n", wifi_scan_iter->ssid, wifi_scan_iter->bssid, wifi_scan_iter->rssi, wifi_scan_iter->ap_auth_type, wifi_scan_iter->ap_crypto_type); wifi_scan_iter = wifi_scan_iter->next;}
						 WM_TEST_SIGNAL;}

						 int rr_connect_ap(char *ssid, char *passwd) {
						 wifi_manager_result_e res = WIFI_MANAGER_SUCCESS; wifi_manager_ap_config_s ap_config; memset(&ap_config, 0, sizeof(ap_config)); strncpy(ap_config.ssid, ssid, WIFIMGR_SSID_LEN); ap_config.ssid_length = strlen(ssid); strncpy(ap_config.passphrase, passwd, WIFIMGR_PASSPHRASE_LEN); ap_config.passphrase_length = strlen(passwd); ap_config.ap_auth_type = RR_AP_AUTH; ap_config.ap_crypto_type = RR_AP_CRYPTO; res = wifi_manager_init(&wifi_callbacks); if (res != WIFI_MANAGER_SUCCESS) {
						 RR_ERR; return -1;}

						 res = wifi_manager_connect_ap(&ap_config); if (res != WIFI_MANAGER_SUCCESS) {
						 RR_ERR; return -1;}
						 else {
						 WM_TEST_WAIT;}
						 ...
						 wifi_manager_info_s info; wifi_manager_result_e res = wifi_manager_get_info(&info); if (res != WIFI_MANAGER_SUCCESS) {
						 RR_ERR; return -1;}
