...
int ws_test_tls_init(struct http_client_tls_t *client, struct http_client_ssl_config_t *ssl_config)
{
	...
	if (ssl_config->dev_cert && ssl_config->private_key) {
		...
		if ((result = mbedtls_x509_crt_parse(&(client->tls_clicert), (const unsigned char *)ssl_config->dev_cert, ssl_config->dev_cert_len)) != 0) {
			PRNT("Error: cli_cert parse fail, returned -%4x", -result);
			goto TLS_INIT_EXIT;
		}

		if ((result = mbedtls_pk_parse_key(&(client->tls_pkey), (const unsigned char *)ssl_config->private_key, ssl_config->private_key_len, NULL, 0)) != 0) {
	...
	if (ssl_config->root_ca) {
		...
		if ((result = mbedtls_x509_crt_parse(&(client->tls_rootca), (const unsigned char *)ssl_config->root_ca, ssl_config->root_ca_len)) != 0) {
