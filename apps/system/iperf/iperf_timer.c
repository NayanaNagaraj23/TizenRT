...
** matter too much.
*/
static void getnow(struct timeval *nowP, struct timeval *nowP2)
{
	if (nowP != NULL) {
		*nowP2 = *nowP;
	} else {
...
static void list_add(Timer * t)
{
	...
	if (timers == NULL) {
		...
		if (t->time.tv_sec < timers->time.tv_sec || (t->time.tv_sec == timers->time.tv_sec && t->time.tv_usec < timers->time.tv_usec)) {
...
struct timeval *tmr_timeout(struct timeval *nowP)
{
	...
	usecs = (timers->time.tv_sec - now.tv_sec) * 1000000LL + (timers->time.tv_usec - now.tv_usec);
	if (usecs <= 0) {
		usecs = 0;
	}
