...
#include "iperf_tcp_window_size.h"
...
int iperf_get_no_fq_socket_pacing(struct iperf_test *ipt)
{
...
void iperf_set_no_fq_socket_pacing(struct iperf_test *ipt, int no_pacing)
{
...
*/
static void mapped_v4_to_regular_v4(char *str)
{
	char *prefix = "::ffff:";
	int prefix_len;
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
	...
	while ((flag = getopt(argc, argv, "p:f:D1VJvsc:t:i:ub:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:")) != -1) {
		switch (flag) {
		case 'p':
		...
		case 'J':
			...
			break;
		case 'v':
			printf("%s\n%s\n%s\n", iperf_version, get_system_info(), get_optional_features());
			exit(0);
		case 's':
			if (test->role == 'c') {
		...
		case 'R':
			...
			client_flag = 1;
			break;
#if defined(HAVE_TCP_WINDOWSIZE)
		case 'w':
		...
		case 'S':
			...
			if (endptr == optarg || test->settings->tos < 0 || test->settings->tos > 255) {
				i_errno = IEBADTOS;
				goto err;
			}
			client_flag = 1;
		...
		case 'A':
			...
			if (endptr == optarg || test->affinity < 0 || test->affinity > 1024) {
				i_errno = IEAFFINITY;
				goto err;
			}
			comma = strchr(optarg, ',');
	...
	if (test->protocol->id == Pudp && blksize > MAX_UDP_BLOCKSIZE) {
	 ...
	 */
	if ((duration_flag && test->settings->bytes != 0) || (duration_flag && test->settings->blocks != 0) || (test->settings->bytes != 0 && test->settings->blocks != 0)) {
...
int iperf_send(struct iperf_test *test, fd_set * write_setP)
{
	...
	for (; multisend > 0; --multisend) {
		...
		SLIST_FOREACH(sp, &test->streams, streams) {
			if (!test->no_fq_socket_pacing || (sp->green_light && (write_setP == NULL || FD_ISSET(sp->socket, write_setP)))) {
				if ((r = sp->snd(sp)) < 0) {
					if (r == NET_SOFTERROR) {
						sched_yield();
						break;
...
static void send_timer_proc(TimerClientData client_data, struct timeval *nowP)
{
...
static int get_parameters(struct iperf_test *test)
{
	...
	if (j == NULL) {
		...
		if ((j_p = cJSON_GetObjectItem(j, "bandwidth")) != NULL) {
			test->settings->rate = (j_p->valueint > 0 ? j_p->valueint : 0);
		}
				...
				for (i = 0; i < n; ++i) {
					...
					if (j_stream == NULL) {
						...
						if (j_id == NULL || j_bytes == NULL || j_retransmits == NULL || j_jitter == NULL || j_errors == NULL || j_packets == NULL) {
							...
							sid = j_id->valueint;
							bytes_transferred = (j_bytes->valueint > 0 ? j_bytes->valueint : 0);
		...
		if (test->protocol->id == Ptcp) {
			if (has_tcpinfo()) {
				...
				if (test->sender && test->sender_has_retransmits) {
					...
					if (rp->stream_min_rtt == 0 || temp.rtt < rp->stream_min_rtt) {
...
static void iperf_print_results(struct iperf_test *test)
{
	...
	iperf_size_t total_sent = 0;
	iperf_size_t bytes_received;
	iperf_size_t total_received = 0;
	...
	if (sp) {
		...
		SLIST_FOREACH(sp, &test->streams, streams) {
			...
			if (sp->diskfile_fd >= 0) {
				if (stat(test->diskfile_name, &sb) == 0) {
...
int iperf_setaffinity(struct iperf_test *test, int affinity)
{
	...
	if (cpuset_getaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(cpuset_t), &test->cpumask) != 0) {
		i_errno = IEAFFINITY;
		return -1;
	}
	...
	if (cpuset_setaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(cpuset_t), &cpumask) != 0) {
		i_errno = IEAFFINITY;
		return -1;
	}
	return 0;
...
int iperf_clearaffinity(struct iperf_test *test)
{
	...
	return 0;
#elif defined(HAVE_CPUSET_SETAFFINITY)
	if (cpuset_setaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(cpuset_t), &test->cpumask) != 0) {
		i_errno = IEAFFINITY;
		return -1;
	}
	return 0;
