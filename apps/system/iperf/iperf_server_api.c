...
void iperf_test_reset(struct iperf_test *test)
{
	struct iperf_stream *sp;
...
static void server_stats_timer_proc(TimerClientData client_data, struct timeval *nowP)
{
...
static void server_reporter_timer_proc(TimerClientData client_data, struct timeval *nowP)
{
	struct iperf_test *test = client_data.p;
...
static void server_omit_timer_proc(TimerClientData client_data, struct timeval *nowP)
{
	struct iperf_test *test = client_data.p;
...
int iperf_run_server(struct iperf_test *test)
{
	...
	if (test->json_output) {
		cJSON_AddItemToObject(test->json_start, "version", cJSON_CreateString(iperf_version));
			...
			if (test->state == CREATE_STREAMS) {
				if (FD_ISSET(test->prot_listener, &read_set)) {
					...
					if (!is_closed(s)) {
						 ...
						 */
						if (test->protocol->id != Pudp || !test->sender) {
							setnonblocking(s, 1);
						}
